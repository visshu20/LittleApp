...
$content = json_encode(array('fetch' => 'disabledProducts'), JSON_FORCE_OBJECT);
...
<div id="mydiv">
    <section class="section">
        ...
        <div id="disabledproductinfo" class="section-body">
            <div class="card">

                <div class="card-body">
                    <div class="form-group fa-pull-left">
                        <input class="form-control py-2 bor" type="search" onkeyup="search('disabledproductsearch','tbldisabledproducts','1')" placeholder="search" id="disabledproductsearch">
                    </div>

                    <table class="table" id="tbldisabledproducts">
                        <thead>
                            <tr>
                                <th>Product Image</th>
                                <th>Name</th>
                            ...
                            <?php   } else { ?>
                                ...
                                <?php foreach ($response as $row) { ?>
                                    <tr id="rec-1">
                                        ...
                                        <td>
                                            <label class="switch">
                                                <input type="checkbox" id="onoffswitch_<?= $row['id'] ?>" name="onoffswitchdisabled" value="<?= $row['availability'] ?>" class="js-switch" <?= $row['availability'] == '1' ? 'checked' : ''; ?> />
                                                <span class="slider round"></span>
                                            </label>
                                            <a onclick="GetUserDetails('<?= @$row['id']; ?>','disablededit')" class="ml-3" id="editproduct" name="Product_<?= @$row['id']; ?>">
    ...
    <form id="disabledproductform" class="needs-validation" novalidate="">
        <div class="modal fade bd-example-modal-lg" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header" style="border-bottom: 1px solid #dee2e6;">
                        <h5 class="modal-title" id="exampleModalLongTitle">Disabled Product</h5>
...
<script>
    $(document).ready(function() {
        ...
        $("#savechanges").click(function() {
            if ($("#disabledproductform").valid()) {
                createproduct('disablededit');
            }
        });
        var validator = $("#disabledproductform").validate({
            rules: {
        ...
        $('input[name=onoffswitchdisabled]').click(function() {
